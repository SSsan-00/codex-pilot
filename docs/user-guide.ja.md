...
暗黙起動では、調査不要と明確な低リスクの文言編集・既知の簡単な説明は対象外にします。通常開発、不明点がある作業、セキュリティやデータ整合性に関わる作業は対象です。`$codex-pilot`と明示した場合は軽作業でも使用します。起動判断のシミュレーションで確認した範囲であり、総tokenや実装品質の効果は未実測です。
...
- 最終変更に対する根拠で受入条件を確認した
- 必要な分析を通常の手段または十分な代替手段で満たした（分析不要ならProviderも不要）
- 重要な正確性・能力不足の問題が未解決で残っていない

検証結果は成功・失敗・未実行・明示的な省略を区別します。Userが省略しても成功の証拠にはならず、既知の失敗や重大な未確認事項は解消されません。省略時は「実装済み・検証省略」のように範囲を限定します。

条件が満たせず有用な作業が残る場合は `Status: partial`、安全に進める作業がない場合は `Status: blocked` とし、不足条件と次の一手を示します。確認済みの部分成果は報告できますが、全体完了と誤解させません。`show_routing = false`でも制約は報告します。Workerの自己申告だけでは検証にならず、Parentのモデル情報が不明なだけでは停止しません。

同じ方針・根拠の不要な再読を避け、変更の影響を受けた検証だけを再実行します。これは指示による防御で、絶対的な誤報防止やtoken削減を保証するものではありません。
...
小変更は対象確認、通常変更は関連テスト、複雑な変更は必要な範囲の追加検証、重大変更は重要な前提の独立確認を行います。Workerは独立作業の分離・並列化が有益な場合だけ使用し、独立した調査1件につき同時に最大1体、タスク全体の既定上限は再試行を含め2体です。調査があるだけでWorkerが必須になるわけではありません。
...
Parent昇降格提案、独自強度番号、多段階昇格、Ponytail必須化は廃止しました。未解決の意味ある失敗に対して、根拠のある強い実行での再試行は1回までです。この制限に通常の修正・影響テストの再実行は含みませんが、ユーザーの作業・予算制限は守ります。Ponytailは任意の補助として利用できます。
