...
When `show_routing` is false, keep the route internal. When true, end the final response with exactly one standalone line containing class, logical family/effort, family-relative strength label, and a reason category. When Pilot started explicit workers, append `| Workers:` and a comma-separated entry for each: `<role>=<exact host-reported model ID> / <reasoning> [<family-stage>]`. Include only workers actually started, preserve their observed metadata, and use `unknown` for any field the host did not expose. Omit `Workers:` when no explicit worker ran; do not claim visibility into an Ultra host's unreported internal fan-out. Do not use the formatted `Routing:` line as a progress update; its required location is the final response. Route-only mode instead returns its compact decision summary and never exposes hidden chain-of-thought.
