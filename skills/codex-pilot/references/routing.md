...
For critical work, independently check security, rollback/data preservation, or concurrency assumptions as applicable; a distinct check or evidence source can suffice without another agent. Preserve failure evidence. Reuse passed checks while relevant code, dependencies, and assumptions remain unchanged; after changes, rerun affected checks rather than automatically repeating the whole suite. Apply the Skill's completion gate and one-retry limit.
