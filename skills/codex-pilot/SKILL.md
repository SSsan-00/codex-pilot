...
Correct obvious command or syntax errors directly. After a meaningful unresolved failure, allow one stronger bounded retry only when evidence suggests it can help and user/agent limits permit it. This limit applies to stronger-capability retries, not ordinary fixes and affected-check reruns within user limits. Carry facts, changed files, failed checks, and the next hypothesis; do not restart a mechanical model ladder. If the reliable floor still cannot be met, report the limitation and unfinished checks; continue safe useful work, but do not claim completion of blocked work. Do not routinely request Parent upgrades or downgrades.
